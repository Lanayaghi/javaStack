...
<body>
	...
	<div class="container">
		...
		<div>
			<h1 class="subtitle">
				Instructor:
				<c:out value="${course.firstName}" />
			</h1>
			<h1 class="subtitle">Sign Ups: ${course.joining.size() }</h1>
		</div>
		...
		<table class="table is-fullwidth is-bordered">
			...
			<tbody>
				...
				<c:forEach items="${course.joining}" var="user">
					<tr>
						<td>${user.name}</td>
						...
						<c:when test="${user.getId() == currentUser.getId()}">
							<c:choose>
							<c:when test="${course.joining.indexOf(currentUser)!= -1}">
